...
//asignar será la Verde. El aula Amarilla no puede ser asignada porque la cantidad
//de personas es menor a la cantidad de bancos disponibles.
//El aula Azul es descartada porque quedan más bancos inutilizados
//que en el aula Verde (6 versus 1).

let aulaAzul: number = 40;
let aulaVerde: number = 35;
let aulaAmarilla: number = 30;

let infantes: number = Number(prompt("ingrese infantes "));

if (infantes < 31) {
  aulaAmarilla = aulaAmarilla - infantes;
  console.log(
    "El curso asignado es la sala amarilla, quedan " +
      aulaAmarilla +
      " lugar vacios."
  );
} else if (infantes < 36) {
  aulaVerde = aulaVerde - infantes;
  console.log(
    "El curso asignado es la sala Verde, quedan " + aulaVerde + " lugar vacios."
  );
} else {
  aulaAzul = aulaAzul - infantes;
  console.log(
    "El curso asignado es la sala Azul, quedan " + aulaAzul + " lugar vacios."
  );
}
